...
% arithmetic rather than against a picture.
    ...
    % The limit acts on what the PLANT receives (uPlant), not on the
    % controller's command (u): the controller may keep asking for 75 while
    % the actuator delivers 20. Checking u would measure the wrong channel.
    k2 = round(2.0 / dt) + 1;
    T = simlab_tests.ok(T, max(r.uPlant(k2:end)) <= 20 + 1e-9, ...
        'the actuator limit event at t = 2 s is in force afterwards (max uPlant = %.4g)', ...
        max(r.uPlant(k2:end)));
    T = simlab_tests.ok(T, max(r.uPlant(1:k2 - 1)) > 20, ...
        'and the delivered command really did exceed 20 before the event (%.4g)', ...
        max(r.uPlant(1:k2 - 1)));
    ...
    T = simlab_tests.ok(T, ~isempty(strfind(txt, '5 event')), 'transcript states the event count');
