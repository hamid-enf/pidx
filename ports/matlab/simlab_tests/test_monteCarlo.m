...
% the study is not working.
    ...
    if logical(rZ.metrics.stable)
        msg = sprintf(['ZN is faster on the PERFECT model (IAE %.4g vs %.4g): ' ...
               'the trade-off the study is built around'], ...
               rZ.metrics.iae, rG.metrics.iae);
        T = simlab_tests.ok(T, rZ.metrics.iae < rG.metrics.iae, '%s', msg);
    end
