...
% not contract.
    ...
    if haveRef
        ...
        T = simlab_tests.near(T, res.model.ku, ...
            simlab_tests.refGet(T, 'relay.ku'), 1e-9, 'identified Ku');
        T = simlab_tests.near(T, res.model.pu, ...
            simlab_tests.refGet(T, 'relay.pu'), 1e-9, 'identified Pu');
        T = simlab_tests.eq(T, double(res.model.quality), ...
            simlab_tests.refGet(T, 'relay.quality'), 'quality score');
        T = simlab_tests.near(T, res.amplitude, ...
            simlab_tests.refGet(T, 'relay.amplitude'), 1e-9, 'mean half-amplitude');
        T = simlab_tests.near(T, res.period_spread, ...
            simlab_tests.refGet(T, 'relay.periodSpread'), 1e-9, 'period spread');
        T = simlab_tests.near(T, res.amp_spread, ...
            simlab_tests.refGet(T, 'relay.ampSpread'), 1e-9, 'amplitude spread');
        T = simlab_tests.near(T, res.asymmetry, ...
            simlab_tests.refGet(T, 'relay.asymmetry'), 1e-9, 'asymmetry');
        ...
        T = simlab_tests.near(T, res.gains.kp, ...
            simlab_tests.refGet(T, 'relay.kp'), 1e-9, 'Tyreus-Luyben Kp');
        T = simlab_tests.near(T, res.gains.ki, ...
            simlab_tests.refGet(T, 'relay.ki'), 1e-9, 'Tyreus-Luyben Ki');
        T = simlab_tests.near(T, res.gains.kd, ...
            simlab_tests.refGet(T, 'relay.kd'), 1e-9, 'Tyreus-Luyben Kd');
        T = simlab_tests.near(T, res.gains.tf, ...
            simlab_tests.refGet(T, 'relay.tf'), 1e-9, 'derivative filter Tf');
    else
    ...
    if haveRef
        T = simlab_tests.near(T, res2.model.k, ...
            simlab_tests.refGet(T, 'steptune.k'), 1e-9, 'step-identified K');
        T = simlab_tests.near(T, res2.model.t, ...
            simlab_tests.refGet(T, 'steptune.t'), 1e-9, 'step-identified T');
        T = simlab_tests.near(T, res2.model.l, ...
            simlab_tests.refGet(T, 'steptune.l'), 1e-9, 'step-identified L');
        T = simlab_tests.eq(T, double(res2.model.quality), ...
            simlab_tests.refGet(T, 'steptune.quality'), 'step quality score');
        T = simlab_tests.near(T, res2.gains.kp, ...
            simlab_tests.refGet(T, 'steptune.kp'), 1e-9, 'AMIGO-step Kp');
        T = simlab_tests.near(T, res2.gains.ki, ...
            simlab_tests.refGet(T, 'steptune.ki'), 1e-9, 'AMIGO-step Ki');
        T = simlab_tests.near(T, res2.gains.kd, ...
            simlab_tests.refGet(T, 'steptune.kd'), 1e-9, 'AMIGO-step Kd');
    else
